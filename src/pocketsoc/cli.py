...
def scan_run(data_dir: str | None = typer.Option(None), profile: Literal["quick", "standard", "deep"] = typer.Option("standard"), resource_profile: Literal["low", "balanced", "high"] = typer.Option("balanced"), time_profile: Literal["auto", "day", "night"] = typer.Option("auto"), since_last: bool = typer.Option(False), parallel: bool = typer.Option(False), timeout: int = typer.Option(60), quiet: bool = typer.Option(False), output: Literal["table", "json", "ndjson"] = typer.Option("table"), fail_on_alert: Literal["none", "low", "medium", "high"] = typer.Option("none"), redact: bool = typer.Option(False), notify: bool = typer.Option(True)) -> None:
    ...
    if not quiet:
        if output == "json":
            ...
            typer.echo(json.dumps(scan_payload)); typer.echo(json.dumps(alerts_payload))
        else:
            ...
            render_scan(hydrated); render_alerts(hydrated)
...
def scan_dashboard(data_dir: str | None = typer.Option(None)) -> None:
    root = _resolve_data_dir(data_dir); scan_data = load_last_scan(root)
    from pocketsoc.models import Alert, CheckResult, ScanResult
    ...
    render_dashboard_summary(hydrated); render_scan(hydrated)
...
def scan_alerts(data_dir: str | None = typer.Option(None), as_json: bool = typer.Option(True, "--json/--table"), redact: bool = typer.Option(False)) -> None:
    root = _resolve_data_dir(data_dir); scan_data, payload = load_last_scan(root), load_alerts(root)
    if redact: scan_data, payload = redact_scan_data(scan_data, payload)
    if as_json: console.print_json(data=payload); return
    from pocketsoc.models import Alert, CheckResult, ScanResult
...
def scan_search(query: str, since: str = typer.Option("7d"), data_dir: str | None = typer.Option(None)) -> None:
    console.print_json(data=global_search(query=query, since=since, data_dir=_resolve_data_dir(data_dir)))
...
def scan_triage_apply(alert_id: str, severity: str, status: str = typer.Option("investigating"), owner: str = typer.Option(""), comment: str = typer.Option(""), source_check: str = typer.Option(""), data_dir: str | None = typer.Option(None)) -> None:
    guard_mutation(); console.print_json(data=upsert_alert_state(alert_id, severity, status=status, owner=owner, comment=comment, source_check=source_check, data_dir=_resolve_data_dir(data_dir)))
...
def scan_triage_template(alert_id: str, severity: str, template: str = typer.Option("investigation_started"), status: str = typer.Option("investigating"), owner: str = typer.Option(""), source_check: str = typer.Option(""), data_dir: str | None = typer.Option(None)) -> None:
    guard_mutation(); console.print_json(data=apply_template(alert_id=alert_id, severity=severity, template_key=template, status=status, owner=owner, source_check=source_check, data_dir=_resolve_data_dir(data_dir)))
...
def report_md(data_dir: str | None = typer.Option(None), redact: bool = typer.Option(False), report_format: Literal["full", "executive"] = typer.Option("full", "--format"), compare_baseline: bool = typer.Option(False, "--compare-baseline")) -> None:
    root = _resolve_data_dir(data_dir); scan_data, alerts = load_last_scan(root), load_alerts(root)
    if redact: scan_data, alerts = redact_scan_data(scan_data, alerts)
...
def report_trends(data_dir: str | None = typer.Option(None), csv: bool = typer.Option(False, "--csv")) -> None:
    root = _resolve_data_dir(data_dir); hist = load_scan_history(root); render_trends(hist)
    if csv: print(f"[green]CSV exported:[/green] {export_trends_csv(hist, root)}")
...
def report_incident_pack(data_dir: str | None = typer.Option(None)) -> None:
    guard_mutation(); print(f"[green]Incident pack written:[/green] {export_incident_pack(_resolve_data_dir(data_dir))}")


@report_app.command("bundle")
def report_bundle(data_dir: str | None = typer.Option(None), since: str = typer.Option("24h"), redact: bool = typer.Option(False, "--redact"), sign: bool = typer.Option(True, "--sign/--no-sign"), encrypt_password: str = typer.Option("", "--encrypt-password")) -> None:
    guard_mutation(); root = _resolve_data_dir(data_dir); _ = since
    out = build_incident_bundle(root, redact=redact, encrypt_password=encrypt_password)
    if sign: print(f"[green]Bundle created:[/green] {out}\n[green]Signature:[/green] {sign_bundle(out, root)}")
    else: print(f"[green]Bundle created:[/green] {out}")


@report_app.command("bundle-decrypt")
def report_bundle_decrypt(bundle_path: str, password: str = typer.Option(..., "--password")) -> None:
    guard_mutation(); print(f"[green]Decrypted bundle:[/green] {decrypt_file(Path(bundle_path), password)}")


@report_app.command("bundle-verify")
def report_bundle_verify(bundle_path: str, data_dir: str | None = typer.Option(None)) -> None:
    if not verify_bundle(Path(bundle_path), _resolve_data_dir(data_dir)): raise typer.Exit(code=2)
...
def baseline_create_cmd(data_dir: str | None = typer.Option(None), profile: Literal["quick", "standard", "deep"] = typer.Option("standard"), device_id: str = typer.Option("local-device")) -> None:
    guard_mutation(); print(f"[green]Baseline saved:[/green] {create_baseline(_resolve_data_dir(data_dir), profile, device_id=device_id)}")
...
def baseline_diff_cmd(data_dir: str | None = typer.Option(None), tolerance: int = typer.Option(0), explain: bool = typer.Option(False)) -> None:
    root = _resolve_data_dir(data_dir); console.print_json(data=diff_baseline(load_last_scan(root), load_baseline(root), tolerance=tolerance, explain=explain))
...
def baseline_policy_eval(data_dir: str | None = typer.Option(None), enforce: bool = typer.Option(False, "--enforce"), baseline_aware: bool = typer.Option(False, "--baseline-aware")) -> None:
    res = eval_policy(_resolve_data_dir(data_dir), baseline_aware=baseline_aware); console.print_json(data=res)
    if enforce and not res.get("compliant", False): raise typer.Exit(code=2)
...
def api_key_rotate_cmd(data_dir: str | None = typer.Option(None)) -> None:
    guard_mutation(); print(f"[green]API token rotated:[/green] {rotate_api_token(_resolve_data_dir(data_dir))}")
...
def maint_verify_integrity(data_dir: str | None = typer.Option(None)) -> None:
    root = _resolve_data_dir(data_dir); s, a = load_last_scan(root), load_alerts(root)
    if s.get("integrity_error") or a.get("integrity_error"): raise typer.Exit(code=2)
...
def maint_forensics_lite(data_dir: str | None = typer.Option(None)) -> None:
    guard_mutation(); root = _resolve_data_dir(data_dir) or Path.home() / ".pocketsoc"
...
def maint_forensics_verify(snapshot_path: str, data_dir: str | None = typer.Option(None)) -> None:
    if not verify_forensics_snapshot(Path(snapshot_path), _resolve_data_dir(data_dir)): raise typer.Exit(code=2)
...
def maint_history_prune(data_dir: str | None = typer.Option(None), days: int | None = typer.Option(None), max_scans: int | None = typer.Option(None), dry_run: bool = typer.Option(False, "--dry-run")) -> None:
    if dry_run:
        hist = load_scan_history(_resolve_data_dir(data_dir)); console.print_json(data={"dry_run": True, "current_entries": len(hist), "days": days, "max_scans": max_scans}); return
    guard_mutation(); print(f"[green]History pruned. Entries kept:[/green] {prune_history(_resolve_data_dir(data_dir), days=days, max_scans=max_scans)}")
...
def maint_archive_rotate(data_dir: str | None = typer.Option(None), dry_run: bool = typer.Option(False, "--dry-run")) -> None:
    if dry_run: console.print_json(data={"dry_run": True, "would_archive": "scan-history.jsonl"}); return
    guard_mutation(); out = rotate_history_archive(_resolve_data_dir(data_dir)); print(f"[green]Archive created:[/green] {out}" if out else "[yellow]No history to archive.[/yellow]")
...
def maint_schedule_install(data_dir: str | None = typer.Option(None), interval: str = typer.Option("6h")) -> None:
    guard_mutation(); print(f"[green]Schedule installed:[/green] {install_schedule(interval, _resolve_data_dir(data_dir))}")
...
def maint_quality_gate(repo_root: str = typer.Option(".")) -> None:
    result = run_quality_gate(Path(repo_root).resolve()); console.print_json(data=result)
    if not result.get("ok", False): raise typer.Exit(code=2)
...
def maint_self_check(data_dir: str | None = typer.Option(None)) -> None:
    ...
    res = run_self_check(_resolve_data_dir(data_dir)); console.print_json(data=res)
    if not res.get("ok", False): raise typer.Exit(code=2)
...
def config_init(data_dir: str | None = typer.Option(None), force: bool = typer.Option(False), profile: Literal["balanced", "strict", "battery-saver"] = typer.Option("balanced")) -> None:
    guard_mutation(); print(f"[green]Config ready:[/green] {write_default_config(_resolve_data_dir(data_dir), force=force, profile=profile)}")


@config_app.command("rules")
def config_rules(data_dir: str | None = typer.Option(None), force: bool = typer.Option(False)) -> None:
    guard_mutation(); print(f"[green]Rules ready:[/green] {write_default_rules(_resolve_data_dir(data_dir), force=force)}")
...
def config_policy(data_dir: str | None = typer.Option(None), force: bool = typer.Option(False), pack: Literal["home-lab", "strict-mobile", "low-power"] = typer.Option("home-lab", "--pack")) -> None:
    guard_mutation(); print(f"[green]Policy ready:[/green] {write_default_policy(_resolve_data_dir(data_dir), force=force, pack=pack)}")
...
def config_restore_cmd(data_dir: str | None = typer.Option(None)) -> None:
    guard_mutation(); console.print_json(data={"restored": restore_config(_resolve_data_dir(data_dir))})
...
def config_suppress_add(pattern: str, expires_at: str = typer.Option(""), data_dir: str | None = typer.Option(None)) -> None:
    guard_mutation(); print(f"[green]Suppression added:[/green] {add_suppression({'pattern': pattern, 'expires_at': expires_at}, _resolve_data_dir(data_dir))}")
...
def config_suppress_remove(index: int, data_dir: str | None = typer.Option(None)) -> None:
    guard_mutation(); console.print_json(data=remove_suppression(index, _resolve_data_dir(data_dir)))
...
def config_autofix_safe(data_dir: str | None = typer.Option(None), dry_run: bool = typer.Option(False, "--dry-run")) -> None:
    if dry_run: console.print_json(data={"dry_run": True, "would_fix": ["config.json", "rules.json"]}); return
    guard_mutation(); console.print_json(data=run_autofix_safe(_resolve_data_dir(data_dir)))
